...
import { createBundle } from '@krypto-wealth/dev/config/rollup';
import path from 'path';

const pkgs = [
  '@krypto-wealth/hw-ledger',
  '@krypto-wealth/keyring',
  '@krypto-wealth/util',
  '@krypto-wealth/util-crypto'
];
...
const entries = ['hw-ledger-transports', 'networks', 'x-fetch', 'x-global', 'x-randomvalues', 'x-textdecoder', 'x-textencoder', 'x-ws'].reduce((all, p) => ({
  ...all,
  [`@krypto-wealth/${p}`]: path.resolve(process.cwd(), `packages/${p}/build`)
}), {});
